...
const PROFILE_DATA = {
    profile_content: beginCell().storeStringTail("ipfs://sadjfksajd").endCell()
}
const TON_COINS = {
...
describe('ProfileFactory Creation', () => {
    ...
    it('educator create profileFactory', async () => {
        ...
        expect((await profileFactory.getGetProfileFactoryData()).owner_address.toString()).toEqual(educator.address.toString());
    });
    ...
    it('educator deploy profileFactory for ansi', async () => {
        ...
        try {
            await profileFactory.getGetProfileFactoryData();
        } catch (error) {
    ...
    it('create profileFactory and send a lot of ton (ton should returned)', async () => {
        ...
        expect((await blockchain.getContract(profileFactory.address)).balance).toEqual(TON_COINS.minTonsForStorage);
        expect((await educator.getBalance())).
...
describe('ProfileFactory general', () => {
    ...
    it('check all init data', async () => {
        ...
        expect(profileFactoryData.collection_content.toString()).toEqual(PROFILE_FACTORY_DATA.content.toString());
        expect(profileFactoryData.owner_address.toString())
            .toEqual(educator.address.toString());
        expect(profileFactoryData.cost).toEqual(PROFILE_FACTORY_DATA.cost);
    });
});
...
describe('ProfileFactory Withdraw', () => {
    ...
    beforeEach(async () => {
        ...
        await profileFactory.send(
            ...
            {
                $$type: 'ProfileCreate',
                profile_content: PROFILE_DATA.profile_content,
            }
        );
    });
    ...
    it('owner withdraw', async () => {
        ...
        expect((await blockchain.getContract(profileFactory.address)).balance).toEqual(toNano('0.04'));
    ...
    it('not owner withdraw', async () => {
        ...
        const profileFactoryBalanceBefore = await (await blockchain.getContract(profileFactory.address)).balance;
        const withdrawResult = await profileFactory.send(
            notOwner.getSender(),
            { value: TON_COINS.gasConsumption },
            'Withdraw'
        ...
        expect((await blockchain.getContract(profileFactory.address)).balance).toEqual(profileFactoryBalanceBefore);
        expect(await notOwner.getBalance()).toBeGreaterThan(notOwnerBalanceBefore - TON_COINS.gasConsumption);
    });
});

describe('ProfileFactory ProfileCreate', () => {
    let blockchain: Blockchain;
    let educator: SandboxContract<TreasuryContract>;
    ...
    it('enroll to the profileFactory', async () => {
        const studentBalanceBefore = await student.getBalance();
        const profileFactoryCost = (await profileFactory.getGetProfileFactoryData()).cost;
        const enrollmentResult = await profileFactory.send(
            student.getSender(),
            { value: profileFactoryCost },
            {
                $$type: 'ProfileCreate',
                profile_content: PROFILE_DATA.profile_content,
            }
        ...
        expect((studentBalanceBefore - profileFactoryCost) - await student.getBalance()).toBeLessThan(TON_COINS.tollerance);
        ...
        expect(profileData.collection_address.toString()).toEqual(profileFactory.address.toString());
        expect(profileData.owner_address.toString()).toEqual(student.address.toString());

        expect(profileData.individual_content.asSlice().loadStringTail())
            .toEqual(PROFILE_DATA.profile_content.asSlice().loadStringTail())
    });

    it('enroll with less coins than cost', async () => {
        const profileFactoryBalanceBefore = (await blockchain.getContract(profileFactory.address)).balance;
        const profileFactoryCost = (await profileFactory.getGetProfileFactoryData()).cost;
        const enrollmentResult = await profileFactory.send(
            student.getSender(),
            { value: TON_COINS.gasConsumption },
            {
                $$type: 'ProfileCreate',
                profile_content: beginCell().storeStringTail("050302013 | 4124@gmail.com").endCell(),
            }
        );
    ...
    it('enroll with a lot coins (should return)', async () => {
        ...
        const enrollmentResult = await profileFactory.send(
            student.getSender(),
            { value: TON_COINS.gasConsumption + profileFactoryCost + toNano("83284") },
            {
                $$type: 'ProfileCreate',
                profile_content: beginCell().storeStringTail("050302013 | 4124@gmail.com").endCell(),
            }
...
describe('Profile Transfer', () => {
    ...
    it('update content of profile', async () => {
        expect((await blockchain.getContract(profile.address)).accountState?.type.toString())
        .toEqual('active');
